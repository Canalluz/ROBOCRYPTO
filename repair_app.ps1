...
$path = "c:\Users\User\Documents\APP SOFTWARE\APPPC\TRADE\App.tsx"
$content = Get-Content $path -Raw

# Identify the start of return ( in AssetFactoryView
$compStart = $content.IndexOf("const AssetFactoryView")
$returnStart = $content.IndexOf("return (", $compStart)
# Identify the export default App; as an end marker
$exportMarker = $content.IndexOf("export default App;")
# Find the last }; before the export
$lastBraceMatch = [regex]::Matches($content, "};")
$lastBraceIndex = -1
foreach ($match in $lastBraceMatch) {
    if ($match.Index -lt $exportMarker -and $match.Index -gt $compStart) {
        $lastBraceIndex = $match.Index
    }
}

if ($returnStart -ne -1 -and $lastBraceIndex -ne -1) {
    $head = $content.Substring(0, $returnStart)
    $tail = $content.Substring($lastBraceIndex)
    ...
    $finalContent = $head + $newReturn + $tail
    Set-Content $path $finalContent -NoNewline
    Write-Host "Successfully repaired App.tsx"
} else {
